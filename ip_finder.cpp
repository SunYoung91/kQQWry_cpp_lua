...
int gbk2utf8(char *utfstr,const char *srcstr,int maxutfstrlen)
{
    if(NULL == srcstr)
    {
        printf("bad parameter srcstr == null \n");
        return -1;
    }
	
	//获取当前的系统本地化设置
	char* nowConfig = setlocale(LC_ALL,NULL);
    ...
    //将unicode编码转换为utf8编码
    if(NULL == setlocale(LC_ALL,"zh_CN.utf8"))//设置 unicode 转换后的码, 当前为utf8
    {
		setlocale(LC_ALL,nowConfig);
        printf("bad parameter.utf8\n");
        return -1;
    }
...
unsigned long CIpFinder::GetString(std::string &str, unsigned long indexStart) const
{
    ...
    // 读取字符串，直到遇到0x00为止
     while (nChar != 0x00) {
        ...
        // 依次放入用来存储的字符串空间中
	    data[ulCount] = static_cast<char>(nChar);
        ++ulCount;
	...
	#ifdef __linux
		char utfstr[2048];	
		gbk2utf8(utfstr,data,2048);
		str.assign(utfstr, strlen(utfstr));
		//printf("====linux=====\n");
	#else
		str.assign(data, ulCount);
		//printf("====windows=====\n");
	#endif
